    ...
    <div class="offer-head">
      ...
      <div class="price">
        <div class="price-initial">
          ...
          <span v-text="offerSummary.rateInitial + offerSummary.rateYearly"></span>
        </div>
      </div>
    </div>
    <div class="offer-body">
      <v-stepper v-model="step" vertical>
        ...
        <v-stepper-content step="1">
          <div class="stepper-content">
            <v-radio-group v-model="services.client.selectedSize">
              <div v-for="(value, key, index) in services.client.size" :key="'size' + key">
        ...
        <v-stepper-content step="2">
          <div class="stepper-content">
            <v-checkbox v-for="(item,i) in services.advanced.design.items" v-model="item.selected" :key="'design' + i" :label="$t(`${item.label}`) + (item.hasOwnProperty('extra') ? ' (' + $t(`${item.extra}`) + ')' : '')" :hint="$t(`${item.hint}`)" persistent-hint light></v-checkbox>
            <div class="stepper-action">
        ...
        <v-stepper-content step="3">
          <div class="stepper-content">
            <v-checkbox v-for="(item,i) in services.advanced.functionality.items" v-model="item.selected" :key="'functionality' + i" :label="$t(`${item.label}`)" :hint="$t(`${item.hint}`)" persistent-hint light></v-checkbox>
            <div class="stepper-action">
        ...
        <v-stepper-content step="4">
          <div class="stepper-content">
            <v-checkbox v-for="(item,i) in services.advanced.infrastructure.items" v-model="item.selected" :key="'infrastructure' + i" :label="$t(`${item.label}`)  + (item.hasOwnProperty('extra') ? ' (' + $t(`${item.extra}`) + ')' : '')" :hint="$t(`${item.hint}`)" persistent-hint light></v-checkbox>
            <v-checkbox v-for="(item,i) in services.advanced.support.items" v-model="item.selected" :key="'support' + i" :label="$t(`${item.label}`)" :hint="$t(`${item.hint}`)" persistent-hint light></v-checkbox>
            <div class="stepper-action">
              <v-btn flat @click="step = 3">{{ $t('buttons.back') }}</v-btn>
              <v-btn color="primary" @click="step = 5">{{ $t('buttons.continue') }}</v-btn>
            </div>
          </div>
        </v-stepper-content>
          ...
          <div class="stepper-content summary">
            ...
            <v-data-table class="table-offer-recurring" v-bind:headers="headersRecurring" :items="offerSummary.summary.recurring" hide-actions v-show="offerSummary.summary.recurring.length">
              <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-right">
                  <span class="total">{{ props.item.rateYearly }}</span>
                </td>
              </template>
            </v-data-table>
